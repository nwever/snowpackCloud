...
echo "fields        = timestamp TA RH VW ISWR HS PSUM_UNHEATED" >> drylake.smet
echo "units_offset  = 0         0  0  0  0    0  0" >> drylake.smet
...
# With filters:
grep -h ^457 [0-9][0-9][0-9][0-9].csv | mawk -F, '{psum=($5>0)?($5*25.4*0.5):(0.); if(prev_sum>psum) {prev_sum=psum}; print substr($2,1,10) "T" substr($3,1,5), ($7>-50 && $7<50)?($7+273.15):(-999), ($33>2 && $33<=100)?($33/100):(-999), ($30>=0 && $30<=100)?($30*0.44704):(-999), ($32>=0 && $32<1500)?($32):(-999), ($8<0)?(0.):($8*0.0254), psum-prev_sum; prev_sum=psum}' >> drylake.smet
